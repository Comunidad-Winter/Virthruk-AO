...
If Hay_Death = True Then
 Call SendData(ToIndex, UserIndex, 0, "||AutoDeah> Lo siento, ya hay un DeathMatch en curso. Espera a que finalize.~255~0~255~0~0" & FONTTYPE_INFO)
 Exit Sub
End If
If UserList(UserIndex).Stats.GLD < 700000 Then
 Call SendData(ToIndex, UserIndex, 0, "||AutoDeah> Para organizar un DeathMatch, tener 200.000 monedas de oro.~255~0~255~0~0" & FONTTYPE_INFO)
 Exit Sub
End If
 UserList(UserIndex).Stats.GLD = UserList(UserIndex).Stats.GLD - 2000000
 Call SendData(ToIndex, UserIndex, 0, "||AutoDeah> Has iniciado el DeathMatch.~255~0~255~0~0" & FONTTYPE_INFO)
 Call SendData(ToAll, 0, 0, "||AutoDeah> El usuario " & UserList(UserIndex).Name & " ha creado un DeathMatch, para ingresar /PARTICIPAR~255~0~255~0~0" & FONTTYPE_INFO)
 Hay_Death = True
  Call SendData(ToIndex, UserIndex, 0, "||AutoDeah> Has entrado al DeathMatch!~255~0~255~0~0" & FONTTYPE_INFO)
 Death_Cantidad = Death_Cantidad + 1
 UserList(UserIndex).flags.EnDeath = True
 Call WarpUserChar(UserIndex, 6, 50, 50, False)
 Exit Sub
...
If Death_Cantidad = 1 Then
 Dim superoro As Obj
 superoro.Amount = 15000000
     If Not MeterItemEnInventario(UserIndex, superoro) Then Call TirarItemAlPiso(UserList(UserIndex).POS, superoro)
 Call SendData(ToIndex, UserIndex, 0, "||AutoDeah> " & UserList(UserIndex).Name & " ha reclamado su premio por salir victorioso en el DeathMatch!! Felicitaciones!~255~0~255~0~0" & FONTTYPE_INFO)
 Hay_Death = False
 UserList(UserIndex).flags.EnDeath = False
 Death_Cantidad = 0
 Call WarpUserChar(UserIndex, 1, 45, 42, True)
 Call SendData(ToIndex, UserIndex, 0, "||AutoDeah> DeathMatch Finalizado!~255~0~255~0~0" & FONTTYPE_INFO)
Else
 Call SendData(ToIndex, UserIndex, 0, "||AutoDeah> Fraude, no puedes reclamar un trofeo ya que hay mas de 1 usuario en el mapa!!~255~0~255~0~0" & FONTTYPE_INFO)
End If
 Exit Sub
 
 Case "#&"
If UserList(UserIndex).Stats.GLD < 700000 Then
 Call SendData(ToIndex, UserIndex, 0, "||AutoDeah> Para Entrar a un DeathMatch, tener 200.000 monedas de oro.~255~0~255~0~0" & FONTTYPE_INFO)
 Exit Sub
End If
...
If Hay_Death = False Then
 Call SendData(ToIndex, UserIndex, 0, "||AutoDeah> No hay ningun DeathMatch ejecutado.~255~0~255~0~0" & FONTTYPE_INFO)
 Exit Sub
End If
If Death_Cantidad = 8 Then
 Call SendData(ToIndex, UserIndex, 0, "||AutoDeah> No puedes ingresar ya que se llego al cupo maximo.~255~0~255~0~0" & FONTTYPE_INFO)
 Exit Sub
End If
 Call SendData(ToIndex, UserIndex, 0, "||AutoDeah> Has entrado al DeathMatch!~255~0~255~0~0" & FONTTYPE_INFO)
 Death_Cantidad = Death_Cantidad + 1
