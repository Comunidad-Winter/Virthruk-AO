...
If UserList(UserIndex).Stats.Exp >= UserList(UserIndex).Stats.ELU Then
    ...
    If Not EsNewbie(UserIndex) And WasNewbie Then
        If UserList(UserIndex).POS.Map = 2 Or UserList(UserIndex).POS.Map = 49 Then
...
If UserList(UserIndex).flags.EnDeath = True Then
 Call SendData(ToAll, 0, 0, "||AutoDeath> El usuario " & UserList(UserIndex).Name & " ha muerto. Si quedan 1 usuario, el ganador debera reclamar su premio a travez de /RECLAMAR~255~0~255~0~0" & FONTTYPE_INFO)
